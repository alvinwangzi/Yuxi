...
本 Skill 的 SKILL.md 随 Agent 平台分发，但生成器（含 12 套主题、1020 个版式、渲染脚本）需首次使用时在沙盒中安装：

```bash
npx -y dashi-ppt-skill@latest --dir ~/.agents/skills
```

国内网络加速：

```bash
npx -y dashi-ppt-skill@latest --dir ~/.agents/skills --registry=https://registry.npmmirror.com
```

安装完成后，Skill 根目录（下文 `<skill-root>`）为：

```
~/.agents/skills/dashi-ppt
```

即 `/home/gem/.agents/skills/dashi-ppt`。

安装是幂等的，重复运行会原地更新（已装依赖自动保留）。安装完成后无需再次安装，除非需要更新版本。

**环境要求**：Node.js 20+ 和 npm；导出 PPTX / PDF 需要本机装有 Chrome / Chromium / Edge。

如果安装失败，检查网络或换用镜像源。如果 `~/.agents/skills/dashi-ppt/project/` 已存在且包含 `node_modules/`，说明之前已安装成功，可跳过安装步骤。

## Skill 目录

`<skill-root>` = `/home/gem/.agents/skills/dashi-ppt`
...
- Deck 语言跟随用户沟通语言：非中文用户在 `goal.json` 顶层加 `"language": "en"`；全部文案字段用目标语言撰写，页面自带的默认中文文案（含结尾页"感谢阅读"类装饰字段）一律覆盖，不得残留中文。
...
- HTML 交付：给用户的预览地址只给 `http://127.0.0.1:<port>/`（不给 https 或 .local 变体）；本机 HTTP 可导出 HTML/PDF/PPTX，本地 HTML 或 `file://` 不能导出可编辑 PPTX。不要返回 `theme-preview`。在自带浏览器的 Agent APP 里生成时，提醒用户导出 PDF/PPTX 前把该地址在系统浏览器中打开。
...
单版式草稿的 `cover` 只能从当前主题前 5 页选择；标准 PageContentPack 3+1 按上文的真实真实候选承载规则选择同质模板池。`image` / `media` 候选基于真实 `mediaSlots`，不是页面标题关键词。动态背景页可用 `ambient` 作为氛围页或章节页。
...
- 导出 PPTX 文件保留编辑能力，可在 PowerPoint 或 WPS 中继续编辑
