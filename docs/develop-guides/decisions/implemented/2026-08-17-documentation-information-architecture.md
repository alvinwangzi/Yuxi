...
文档按“入门教程 → 配置与操作参考 → 机制详解 → 开发治理 → 决策与事故”组织。教程提供有顺序、可观察结果的学习路径，参考页集中当前可查询事实；一个页面出现实质性混合职责时拆分。`docs/AGENTS.md` 保存修改文档时必须执行的短指令，完整写作标准由[文档编写规范](../../documentation-guidelines.md)拥有，其他页面只链接、不复制规则。

根 `AGENTS.md` 按只读分析、环境操作、实现、非平凡变更、提交和并行环境提供加载入口；已加载且未变化的材料无需重复读取。子树规则只补充当前目录的要求，单纯阅读材料不触发实现、决策写作或提交流程。测试层级、最低证据与提交前命令集中由[测试规范](../../testing-guidelines.md)维护；通用技能复用项目的决策和验证体系，同类任务选择一套主流程，提交与外部副作用仍需当前任务授权。规范定义应守约束，当前装配和直接证据证明实际行为，二者不相互替代。
...
- 提交前命令和后端 lint 集中由测试规范维护，子树规则和贡献指南使用指针引用，不再复制命令块；新增或修改命令只需更新一处。
...
| 规则去重后 AGENTS 字符预算、链接和 verifier 无新增错误 | 工程契约检查与基线对比；verifier unit；字符数统计 | 62 个 verifier unit 在 Linux 通过（Windows 1 个路径差异为既有问题）；四个 AGENTS 文件均在字符预算内；基线 20 个既有错误未扩大；docs build 因 pnpm 版本不匹配未运行 |
