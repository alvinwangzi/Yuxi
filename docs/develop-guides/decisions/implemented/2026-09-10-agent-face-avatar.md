...
`web/src/utils/agentFaceAvatar.js` 拥有智能体默认头像的参数决策、企业场景词典与 SVG 生成。它以 name 与 description 拼接串的确定性哈希为 seed 生成 64×64 Notion 风格人像 SVG data URI：配饰按「标题优先、描述其次」匹配企业场景词典，多词命中按词典固定类别顺序取首个，无命中时哈希决定（含无配饰）；肤色、发型、背景色由哈希从参数池选取。词典围绕十三种配饰组织为十三类场景域：审核与合规（check）、开发工程（terminal）、设计创意（palette）、营销传播（megaphone）、销售与增长（target）、外部信息与检索（globe）、写作与文档（pen）、数据与量化（chart）、财务与会计（coins）、调研与归档（clipboard）、人事与行政（users）、研究与咨询（glasses）、领导与管理（briefcase）；初始词表参考 The Agency 智能体角色清单对照企业常见部门扩充，覆盖客服、人事、财务、法务、运维、销售、运营、工程开发、设计、营销、管理等企业常见智能体角色；check 置于 terminal 前使「测试工程师」「安全工程师」仍归质检，而「前端工程师」「算法工程师」落入开发工程；target、coins、users 从相邻通用配饰拆出独立场景域（target 置于 globe 前、coins 置于 chart 后、users 置于 clipboard 后），chart 只收数据与量化职能，使销售、财务、人事岗位各有可辨识配饰，交叉词按固定顺序裁决（财务报表归 chart、薪酬核算归 coins、电商客服归 target）；briefcase 置于最后使纯管理身份词（CEO、总监、经理）落入领导管理，含业务领域词的名称先归业务域（销售总监归 target、首席财务官归 coins）。完整词表由 `agentFaceAvatar.js` 拥有并在单测中逐条断言，本记录维护词表归属与组织原则，词表条目本身随测试演进。
...
- `cd web; pnpm test:unit`：286 个测试，283 通过，3 失败均为预存问题并与本次改动无关——`api_boundary.test.js:89`（401 会话清理断言）、`database_create_flow.test.js:62`（知识库类型标签期望）、`projectConversationGroups.test.js:86`（读取用户在途未提交的 `ConversationNavSection.vue` 源文本）。`agentFaceAvatar.test.js` 10/10 通过（确定性、词典全部关键词逐条断言命中各自配饰、标题优先裁决、工程/设计/营销角色命中与质检词优先顺序、销售/财务/人事岗位命中独立配饰与交叉词裁决、管理身份命中公文包与业务词优先顺序、兜底取值合法、空输入空串、data URI 结构、新增配饰 SVG 特征断言），`dashboard_thread_stats.test.js` 断言更新后通过。
...
- 真实页面验证（Vite dev server localhost:5173，登录用户 alvin）：
  ...
  - 新建智能体弹窗（领导管理类扩充后复验）：输入「CEO 战略助手」预览为带公文包配饰（briefcase：包体、提手与扣带线）的人像；「技术总监」命中 briefcase。
  - 新建智能体弹窗（销售/财务/人事拆分类目后复验）：输入「销售总监」预览为带靶心配饰（target：同心三环）的人像；「财务总监」命中硬币（coins：三枚叠放椭圆）；「人事专员」命中双人形（users：前后双头双肩）；「数据分析」仍命中柱状图（chart），数据量化职能与销售、财务、人事岗位互不混淆。
...
- 负向案例：词典全部关键词逐条断言命中各自类别（含工程、设计、营销、管理扩充词）；「写作助手」+「数据统计」标题优先命中 pen；「财务分析助手」命中 chart；「测试工程师」含「工程师」（terminal）仍命中 check，证明词典顺序裁决；「销售总监」含「总监」（briefcase）命中 target，证明业务词优先于管理词；「财务报表」含「财务」（coins）仍命中 chart，「薪酬核算」含「薪酬」（users）命中 coins，「电商客服」含「客服」（globe）命中 target，证明拆分后交叉词按固定顺序裁决；无词典命中输入输出合法配置；空输入返回空串并落入 FallbackAvatar 首字母兜底；新建态清空名称回空态（is-empty 生效）；审计表断言同时覆盖智能体列切换与既有 uid 列保持。
